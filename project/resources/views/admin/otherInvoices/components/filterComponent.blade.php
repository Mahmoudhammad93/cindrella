<div class="row">
    ...
    <form action="{{ route($buttonsRoutsname.'.index') }}" method="GET">
        ...
        filterInputForm(['type' => 'text', 'class' => 'form-control','name' => $input ,'placeholder' => trans('main.desc') , 'value' => $inputValue ] , 3 );

        $input ="value";
        isset($filterData[$input]) ? $inputValue = $filterData[$input] : $inputValue = '';
        filterInputForm(['type' => 'number', 'step'=>'any' ,'class' => 'form-control','name' => $input ,'placeholder' => trans('main.value') , 'value' => $inputValue ] , 3 );
