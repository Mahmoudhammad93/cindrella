...
class SellInvoice extends BackEndController
{
    ...
    public function totalgainindex(Request $request){
      ...
      $PageTitle = trans('main.total_gain');
      $headerLevelProcessTitle1 = trans('main.total_gain');
    ...
    public function index(Request $request)
    {
        ...
        $PageTitle = trans('main.sell_invoice');
        $headerLevelProcessTitle1 = trans('main.sell_invoice');
        $headerLevelProcessTitle2 = trans('main.site.all');
...
function getProductInfo (){
 ...
 function invoiceCode(){
    $invoice = Invoice::where('invoice_type','=',1)->where('date','=',date('Y-m-d'))->get()->last();
    $todayInvoiceStart = 0 ;
    $dateNow = now()->format('Y-m-d');

    if ($invoice){
      if ($invoice->date === $dateNow){
          return $invoice->code;
      }
    }
 }
