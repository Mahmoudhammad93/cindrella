...
class PurchaseInvoice extends BackEndController
{
    ...
    public function index(Request $request)
    {
        ...
        $PageTitle = trans('main.pur_invoice');
        $headerLevelProcessTitle1 = trans('main.pur_invoice');
        $headerLevelProcessTitle2 = trans('main.site.all');
        $singlePageTitle = trans('main.payment_invoice');
        $buttonsRoutsname = $modelViewName = "purchaseInvoice";
        $databind  = $this->append();

        return View('Admin.purchaseInvoice.index',compact('filterData','rows','databind','PageTitle','buttonsRoutsname','singlePageTitle','headerLevelProcessTitle1','headerLevelProcessTitle2'));
        ...
        if($invoice = $row->create($request->toArray()) ){
            ...
            for($i=0;$i <= $request['itrator'];$i++){
              ...
              if($request['due'] == 0 ){
                   ...
                   $box->desc = trans('main.instant_payment_invoice');
                   $box->invoiceType  = 1;
                   ...
                   $supplier_balance->desc = trans('main.instant_payment_invoice');
    ...
    public function show($id)
    {
      ...
      $headerLevelProcessTitle1 = trans('main.pur_invoice');
      $headerLevelProcessTitle2 = trans('main.num_invoice').$row->code;
      $buttonsRoutsname = $modelViewName = "purchaseInvoice";
    ...
    function purchaseCode(){
        $invoice = Invoice::where('invoice_type','=',0)->where('date','=',date('Y-m-d'))->get()->last();
        $todayInvoiceStart = 0 ;
        if ($invoice){
            return $invoice->code;
        }
        // $invoice = Invoice::where('invoice_type','=',0)->get()->last();
        // if ($invoice){
        //     return $invoice->code;
        // }
     }
